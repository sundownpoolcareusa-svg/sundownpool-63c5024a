...
// Uses the get_estimate_public SECURITY DEFINER RPC (anonymous, token-gated)
...
export const getPublicEstimate = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { data: result, error } = await supabase.rpc("get_estimate_public", { _token: data.token });
    if (error) throw error;
    if (!result) return null;
    return result as unknown as PublicEstimateRpcResult;
  });

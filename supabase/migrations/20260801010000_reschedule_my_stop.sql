-- Lets a technician reschedule one of their own pending stops to a
-- different date, mirroring the admin's rescheduleStop() but scoped to the
-- calling technician (same ownership pattern as reorder_my_stops /
-- update_my_stop_status — a technician has no direct RLS access to
-- routes/route_stops, so this has to run as a SECURITY DEFINER RPC).
--
-- p_all_future=true additionally swaps the client's recurring service_days
-- from the stop's original weekday to the new one, and clears out any other
-- already-generated future pending stops that no longer match the updated
-- schedule — the same cleanup the admin's client-edit flow does via
-- removeStaleClientStops(), needed here too so changing the schedule from
-- the technician's app doesn't leave stale duplicate stops behind.

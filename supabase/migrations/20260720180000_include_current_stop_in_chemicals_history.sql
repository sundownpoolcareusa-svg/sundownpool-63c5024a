-- The Chemistry/Product History views previously excluded the stop
-- currently being edited, on the assumption "today" didn't belong in
-- "history". Technicians want today's own just-saved entry to show up
-- there too (for confirmation), so this drops that exclusion.
CREATE OR REPLACE FUNCTION public.get_my_stop_chemicals_history(p_stop_id uuid)
RETURNS TABLE (
  route_stop_id uuid,
  route_date date,

-- The previous migration's get_my_stop_detail rewrite (pool/spa support)
-- accidentally dropped filter_last_cleaned_at and filter_cleaning_count from
-- its return columns — they were added later than the version this was
-- based on, so the Filter Cleaning card on the technician's Chemicals page
-- silently lost its data. Restore them alongside has_spa.
...
AS $$
  SELECT rs.id, rs.position, rs.status, c.name, c.address, c.city, c.state, c.zip, c.client_type,
         c.filter_last_cleaned_at, c.filter_cleaning_count, c.has_spa
